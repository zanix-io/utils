...
Deno.test('ValidatorRTO validation', async () => {
  ...
  await assertRejects(
    () =>
      classValidation(ValidatorRTO, { stringPropWithDefaults: 'ds' }).catch(
        (err) => {
          ...
          assertEquals(err.cause.properties.stringPropWithDefaults, [{
            constraints: [
              "'stringPropWithDefaults' must be a valid numeric string.",
            ],
            value: 'ds',
            plainValue: 'ds',
          }])
  ...
  await assertRejects(
    () =>
      classValidation(ValidatorRTO, { value: '3', valueOptional: '4' }).catch(
        (err) => {
          assertEquals(err.cause.message, 'Request validation error')
...
Deno.test('Class validation', async () => {
  // showld not throws
  const values = await classValidation(ValidatorRTO, {}, {
    throwErrors: () => {},
  })
  ...
  const values2 = await classValidation(ValidatorRTO, {
    extraneus: 'extraneus',
  }, {
    throwErrors: () => {},
  ...
  const values3 = await classValidation(ValidatorRTO, {
    extraneus: 'extraneus value',
  }, {
    throwErrors: () => {},
