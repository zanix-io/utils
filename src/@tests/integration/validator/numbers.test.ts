...
Deno.test('Number validations RTO errors', async () => {
  ...
  await assertRejects(
    () =>
      classValidation(NumbersRTO, { stringNumber: 'text', numberValue: '4' })
        .catch((err) => {
          assertEquals(err.cause.message, 'Request validation error')
          assertEquals(Object.keys(err.cause.properties).length, 1)
  ...
  await assertRejects(
    () =>
      classValidation(NumbersRTO, { numberValue: '4', numberA: '3d' }).catch(
        (err) => {
          assertEquals(err.cause.message, 'Request validation error')
          assertEquals(Object.keys(err.cause.properties).length, 1)
  ...
  await assertRejects(
    () =>
      classValidation(NumbersRTO, { numberValue: '4', numbers: [3, '4d'] })
        .catch((err) => {
          assertEquals(err.cause.message, 'Request validation error')
          assertEquals(Object.keys(err.cause.properties).length, 1)
  ...
  await assertRejects(
    () =>
      classValidation(NumbersRTO, { numberValue: '4', maxNumber: 4 }).catch(
        (err) => {
          assertEquals(err.cause.message, 'Request validation error')
          assertEquals(Object.keys(err.cause.properties).length, 1)
      ...
      classValidation(NumbersRTO, {
        ...
        .catch((err) => {
          ...
          assertEquals(err.cause.properties.minNumber, [{
            constraints: [
              "'minNumber' must be a number greater or equal than 3.",
            ],
            value: 2,
            plainValue: '2',
          }])
...
Deno.test('Number validations RTO', async () => {
  ...
  // Escenario 2
  const resp2 = await classValidation(NumbersRTO, {
    numberValue: '4',
    stringNumber: '2',
  })
  ...
  const resp3 = await classValidation(NumbersRTO, {
    numberValue: '4',
    numberA: '3',
  })
  ...
  const resp4 = await classValidation(NumbersRTO, {
    numberValue: '4',
    numbers: ['3', '4', '5'],
  })
  ...
  const resp6 = await classValidation(NumbersRTO, {
    numberValue: '4',
    minNumber: '5',
  })
  ...
  // Number arrays
  const resp7 = await classValidation(NumbersRTO, {
    numberValue: '3',
    numbersDefault: [4, 3, 5],
  })
