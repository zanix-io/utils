...
| `setGlobalZnx` | `(data: Partial<Zanix>): void` | Initializes `Znx` on `globalThis` on first call, then merges `data` into it on every call. `Znx.config` resolves lazily from the project's `deno.json` `zanix` field (ignoring read errors) the first time something actually reads it — not at this call — so merely calling `setGlobalZnx` (e.g. via constructing a `Logger`) never touches disk on its own. |
...
`mockWrap` and `resetConfig` actually live in `modules/testing/mod.ts`, not in
the `helpers` group, but they are the only public symbols exported from that
module, so they are documented here rather than in their own file.
...
| `resetConfig` | `(): void`                                                                      | Clears `readConfig`'s memoized result, forcing its next call to re-read the config file from disk. Test-only — production code relies on that cache staying warm for the process lifetime. Doesn't retroactively affect an already-resolved `Znx.config` (see `setGlobalZnx` above), which resolves independently, once, on its own first access.                                                                                                                                                                                                                                   |
