...
| `MinAge`             | `(years: number, options?: DefaultTransformValidationOpts) => ValidationDecoratorDefinition` | Validates that the value is a `Date` far enough in the past to represent an age of at least `years` (e.g. a birth date). Its threshold is recomputed from `new Date()` on every validation call rather than captured once, so it never drifts however long the process stays up. Same auto-transform/auto-expose behavior as `IsDate`. |
| `minAge`             | `(years: number, value?: Date) => boolean`                                                   | Raw predicate.                                                                                                                                                                                                                                                                                                                         |
| `minAgeArray`        | `(years: number, values: Date[]) => boolean`                                                 | Raw predicate for arrays.                                                                                                                                                                                                                                                                                                              |
| `MaxAge`             | `(years: number, options?: DefaultTransformValidationOpts) => ValidationDecoratorDefinition` | Validates that the value is a `Date` not far enough in the past to represent an age older than `years` (e.g. rejecting an implausible birth date). Same per-call recomputed threshold as `MinAge`, and the same auto-transform/auto-expose behavior as `IsDate`.                                                                       |
| `maxAge`             | `(years: number, value?: Date) => boolean`                                                   | Raw predicate.                                                                                                                                                                                                                                                                                                                         |
| `maxAgeArray`        | `(years: number, values: Date[]) => boolean`                                                 | Raw predicate for arrays.                                                                                                                                                                                                                                                                                                              |
...
class BookingRTO extends BaseRTO {
  ...
  @MinAge(18)
  accessor birthDate!: Date // must represent an age of at least 18 years, re-evaluated on every call

  @MaxAge(120, { optional: true })
  accessor implausibleBirthDate: Date | undefined // rejects a birth date that implies an age over 120 years
}
```
