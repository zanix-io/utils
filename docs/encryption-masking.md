...
digital signatures (RSASSA-PKCS1-v1_5, i.e. "RS256"/etc per RFC 7518 §3.3, or
HMAC for a symmetric-secret signature). Use
...
RSA encryption uses `RSA-OAEP`. RSA signing (`signRSA`/`verifyRSA`) always uses
`RSASSA-PKCS1-v1_5` — real "RS256"/"RS384"/"RS512" per RFC 7518 §3.3 — regardless
of which `algorithm` a keypair was generated with, since a PEM key export
carries no algorithm-specific material.
...
| Function                                          | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `generateRSAKeys(options?)`                       | Generates a PEM-formatted `{ privateKey, publicKey }` pair. `options.hash` (`HashAlgorithm`, excluding `'SHA-1'`) defaults to `'SHA-256'`; `options.modulusLength` (`ValidRSAModulusLength`) defaults to `2048`; `options.algorithm` (`'RSA-OAEP' \| 'RSA-PSS' \| 'RSASSA-PKCS1-v1_5'`) defaults to `'RSA-OAEP'` and only selects the generated key's WebCrypto `keyUsages` — `signRSA`/`verifyRSA` always sign/verify under their own fixed `RSASSA-PKCS1-v1_5` regardless of which value generated the keypair. |
...
| `signRSA(message, privateKey, hash?)`             | Signs a message with an RSA **private** key using `RSASSA-PKCS1-v1_5` (real "RS256"/etc per RFC 7518 §3.3). `hash` defaults to `'SHA-256'`. Returns a `Uint8Array`.                                                                                                                                                                                                                                                                                                                                               |
| `verifyRSA(message, signature, publicKey, hash?)` | Verifies an `RSASSA-PKCS1-v1_5` signature against the original message using the RSA **public** key. Returns a `Promise<boolean>`.                                                                                                                                                                                                                                                                                                                                                                                |
...
| `signHMACBytes(key, data, hash?)`                 | Raw-bytes HMAC signing: both `key` and `data` are `Uint8Array` instead of `signHMAC`'s UTF-8 `string`, and `hash` accepts the full `HashAlgorithm` range including `'SHA-1'` (which `signHMAC` excludes, since JWT has no HS1 algorithm). `hash` defaults to `'SHA-1'`. Use this over `signHMAC` when the key is arbitrary binary (e.g. a TOTP secret) that would be corrupted by round-tripping through a JS string.                                                                                             |
